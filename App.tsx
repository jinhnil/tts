...
const App: React.FC = () => {
  ...
  const [storedProgress, setStoredProgress] = useState<Record<string, StoredProgress>>({});
  const [lastPlayedFile, setLastPlayedFile] = useState<string | undefined>(undefined);
  ...
  const saveProgress = useCallback((fileName: string, chunkIndex: number, settings: ReaderSettings) => {
  ...
  return (
    <div className="h-screen w-screen bg-gray-950 overflow-hidden text-white font-sans relative">
      ...
      {!selectedFile ? (
        ...
        <Reader
          file={selectedFile}
          initialSettings={storedProgress[selectedFile.name]?.settings || DEFAULT_SETTINGS}
          initialChunkIndex={storedProgress[selectedFile.name]?.chunkIndex || 0}
          onBack={handleBack}
          onSaveProgress={(chunkIndex, settings) => saveProgress(selectedFile.name, chunkIndex, settings)}
        />
      )}
    </div>
  );
